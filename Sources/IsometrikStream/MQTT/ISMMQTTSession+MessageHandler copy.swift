...
extension ISMMQTTSessionWrapper: CocoaMQTTDelegate {
    ...
    public func mqtt(_ mqtt: CocoaMQTT, didReceiveMessage message: CocoaMQTTMessage, id: UInt16 ) {
        ...
        LogManager.shared.logMQTT("EVENT FOR ACTION: \(actionName)", type: .info)
        LogManager.shared.logMQTT("EVENT MESSAGE: \(eventMessage)")
    ...
    public func mqttDidDisconnect(_ mqtt: CocoaMQTT, withError err: Error?) {
        if err == nil {
            LogManager.shared.logMQTT("", type: .info)
        } else {
            let errorDescription = err?.localizedDescription
            LogManager.shared.logMQTT("ERROR WHILE DISCONNECTION", type: .error)
        }
