...
extension IsometrikStream {
    ...
    public func fetchModerators(streamId: String, skip: Int, limit: Int = 20, searchTag: String? = nil, completionHandler: @escaping (ISMModerator)->(), failure : @escaping (ISMLiveAPIError) -> ()) {
        
        let request =  ISMLiveAPIRequest<Any>(endPoint: ModeratorRouter.fetchModerators(streamId: streamId, searchTag: searchTag, limit: limit, skip: skip) , requestBody:nil)
