...
//
//  Created by Appscrip 3Embed on 16/05/24.
//

import AVFoundation
import UIKit

public enum ISM_VideoQuality {
    case very_high
    case high
    case medium
    case low
...
// Compression Result
public enum ISM_CompressionResult {
    case onStart
    case onSuccess(URL)
    case onFailure(ISM_CompressionError)
    case onCancelled
}

// Compression Interruption Wrapper
public class ISM_Compression {
    public init() {}
...
public struct ISM_CompressionError: LocalizedError {
...
@available(iOS 11.0, *)
public struct ISM_LightCompressor {
    ...
    public func ism_compressVideo(source: URL,
                                  destination: URL,
                                  quality: ISM_VideoQuality,
                                  isMinBitRateEnabled: Bool = true,
                                  keepOriginalResolution: Bool = false,
                                  progressQueue: DispatchQueue,
                                  progressHandler: ((Progress) -> ())?,
                                  completion: @escaping (ISM_CompressionResult) -> ()) -> ISM_Compression {
        
        var frameCount = 0
        let compressionOperation = ISM_Compression()
        ...
        guard let videoTrack = videoAsset.tracks(withMediaType: AVMediaType.video).first else {
            let error = ISM_CompressionError(title: "Cannot find video track")
            completion(.onFailure(error))
            return ISM_Compression()
        }
        ...
        if isMinBitRateEnabled && bitrate <= MIN_BITRATE {
            let error = ISM_CompressionError(title: "The provided bitrate is smaller than what is needed for compression try to set isMinBitRateEnabled to false")
            completion(.onFailure(error))
            return ISM_Compression()
        }
        
        // Generate a bitrate based on desired quality
        let newBitrate = ism_getBitrate(bitrate: bitrate, quality: quality)
        
        // Handle new width and height values
        let videoSize = videoTrack.naturalSize
        let size = ism_generateWidthAndHeight(width: videoSize.width, height: videoSize.height, keepOriginalResolution: keepOriginalResolution)
        ...
        let videoWriterInput = AVAssetWriterInput(mediaType: AVMediaType.video, outputSettings: ism_getVideoWriterSettings(bitrate: newBitrate, width: newWidth, height: newHeight))
        ...
        do{
            ...
            let compressionError = ISM_CompressionError(title: error.localizedDescription)
                                    ...
                                    while audioWriterInput.isReadyForMoreMediaData {
                                        ...
                                        if audioReader?.status == .reading && sampleBuffer != nil {
                                            ...
                                            videoWriter.finishWriting(completionHandler: {() -> Void in
                                                compressionOperation.cancel = true
    ...
    private func ism_getBitrate(bitrate: Float, quality: ISM_VideoQuality) -> Int {
    ...
    private func ism_generateWidthAndHeight(
        width: CGFloat,
        height: CGFloat,
    ...
    private func ism_getVideoWriterSettings(bitrate: Int, width: Int, height: Int) -> [String : AnyObject] {
...
extension URL {
    func ism_verboseFileSizeInMB() -> Float?{
        let p = self.path
